...
const firebaseConfig = {
  ...
  function renderParkingMap() {
    ...
    parkingSlots.filter(slot => slot["car-number"]).forEach(slot => {
        ...
        const parkedTime = calculateParkedTime(slot.parkedAt);
...
// Live Update Parked Time Every Minute
setInterval(() => {
    renderParkingMap();
}, 6000);
  ...
  function createVehicle() {
    ...
    if (slot && !slot["car-number"]) {
        ...
        slot.status = "In-Car";
        slot.parkedAt = Date.now(); // Save timestamp in milliseconds
...
function calculateParkedTime(parkedAt) {
    if (!parkedAt) return "Just parked";
...
function removeCar(parkingNumber) {
  ...
  if (slot && slot["car-number"]) {
      ...
      const removedCarDetails = {
          ...
          "status": slot.status,
          // "parkedTime": calculateParkedTime(parkedAt),
